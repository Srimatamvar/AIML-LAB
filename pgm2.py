class Graph:
    ...
    def computeMinimumCostChildNodes(self, v): # Computes the MinimumCost of child nodes of a given node v
        minimumCost = 0
        costToChildNodeListDict = {}
        costToChildNodeListDict[minimumCost] = []
        flag = True
        for nodeInfoTupleList in self.getNeighbors(v): # iterate overall the set of child node/s
            cost = 0
            nodeList = []
            ...
            if flag == True: # initialize Minimum Cost with the cost of first set of child node/s
                minimumCost = cost
                costToChildNodeListDict[minimumCost] = nodeList # setthe Minimum Cost child node/s
                flag = False
    ...
    def aoStar(self, v, backTracking): # AO* algorithm for a start node and backTracking status flag
        ...
        if self.getStatus(v) >= 0: # if status node v >= 0, compute Minimum Cost nodes of v
            ...
            if solved == True: # if the Minimum Cost nodes of v are solved, set the current node status as solved(-1)
                ...
                # solution
            if v != self.start: # check the current node is the startnode for backtracking the current node value
...
graph2 = { # Graph of Nodes and Edges
    ...
    'B': [[('G', 1)], [('H', 1)]], # Neighbors are included in a listof lists
